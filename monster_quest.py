...
def render_allocate():
    ...
    if st.button("Submit round →", disabled=(remaining < 0), type="primary", key=f"gq_submit_{r}"):
        ...
        st.session_state.gq_phase = "reflect" if r in REFLECTION_ROUNDS else "result"
        st.rerun()

# ── Reflect phase (before results) ───────────────────────────────────────────
def render_reflect():
    r = st.session_state.gq_round
    st.subheader(f"Round {r} — before you see the results")
    st.write(REFLECTION_QUESTIONS[r])

    text = st.text_area(
        "Your answer:",
        height=150,
        placeholder="Write your thinking here — what you did, why, and what you expect to see...",
    ...
    if st.button("See results →", disabled=not ready, type="primary", key=f"gq_reflect_submit_{r}"):
...
def main():
    ...
    elif phase == "reflect":  render_reflect()
